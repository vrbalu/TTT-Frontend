...
export class TttComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.ws.onmessage = (message) => {
      ...
      if (game.user1 === this.currentUser.username) {
        ...
        this.waiting = true;
        this.subscriptions.push(this.userService.updateUser({
          username: game.user1,
          online: true,
          inGame: true
        }).subscribe());
  ...
  join(id: number, user1: string, user2: string): void {
    ...
    this.gameWs.onmessage = (message) => {
      if (message.data.startsWith('WON')) {
        const updateGame: UpdateGame = {id, winner: message.data.substring(4), isPending: false, isFinished: true};
        ...
        const move: Gameplay = JSON.parse(message.data);
        if (move.id === this.currentGamePlay.id) {

          // @ts-ignore
          const x = document.getElementById('myTable').rows[move.x].cells;
  ...
  checkWin(shape: string, move: any): boolean {
    ...
    for (let i = 0; i < lineDirections.length && !won; i++) {
      const shift = lineDirections[i];
      let currentSurrounding = [move[0] + shift[0], move[1] + shift[1]];
      let lineLength = 1;

      while (lineLength < minimumLength && this.legalSquare(currentSurrounding) &&
      this.board[currentSurrounding[0]][currentSurrounding[1]] === shape) {
        lineLength++;
        currentSurrounding[0] += shift[0];
        currentSurrounding[1] += shift[1];
      }

      currentSurrounding = [move[0] - shift[0], move[1] - shift[1]];
      while (lineLength < minimumLength && this.legalSquare(currentSurrounding) &&
      this.board[currentSurrounding[0]][currentSurrounding[1]] === shape) {
        lineLength++;
        currentSurrounding[0] -= shift[0];
  ...
  legalSquare(square: any): boolean {
